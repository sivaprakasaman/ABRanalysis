...
function zzz4onefreq
%% Initialize
global abr_Stimuli abr_data_dir	num dt line_width abr freq attn spl w upper_y_bound lower_y_bound padvoltage y_shift date data noise freq_level han animal abr_time thresh_mag ABRmag invert
...
if exist(hhh.name,'file')
    ...
    for i=1:num
        ...
        if iscell(x.AD_Data.AD_Avg_V)
            abr(:,i)=(x.AD_Data.AD_Avg_V{1}(1:end-1)-mean(x.AD_Data.AD_Avg_V{1}(1:end-1)))'; %#ok<*AGROW> % removes DC offset
        else
            abr(:,i)=x.AD_Data.AD_Avg_V(1:end-1)'-mean(x.AD_Data.AD_Avg_V(1:end-1)); % removes DC offset
        end
    end
...
data.threshold_pwl=(meanbase-b(1,1))/b(2,1);
% w=w1;
...
text(0.04,0.9375,['Chin' char(animal) ' on ' char(strrep(date,'_','-'))],'FontSize',14,'horizontalalignment','left','VerticalAlignment','bottom')
