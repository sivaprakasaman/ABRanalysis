...
global abr_FIG abr_Stimuli ABRmag abr_root_dir abr_data_dir noise
...
abr=zeros(380,num);
for i=1:num
    fname=dir(sprintf('p%04d*',pic(i))); %p or a?
    ...
    command=sprintf('abr(3:380,%d)=transpose(x.AD_Data.AD_Avg_V{1}(1:end-1))',i);
    eval([command ';'])
...
get_noise;
null_xx=xcorr((noise/20000*1000000),template2); %both waves in uV
...
text(0.55,0.934,'uV','fontsize',10,'horizontalalignment','left')
text(0.755,0.934,'Z score','fontsize',10,'horizontalalignment','left')
