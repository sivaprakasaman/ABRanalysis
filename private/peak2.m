...
global abr_Stimuli num abr upper_y_bound lower_y_bound y_shift data abr_time abr padvoltage AR_marker abr_FIG spl replot line_width updated_data freq
...
while x >= abr_Stimuli.start && x <= abr_Stimuli.end
    ...
    if ~isempty(replot)
        ...
        if ~isnan(peaks_sel(i,index))
            replot_check = 1;
        elseif ~isnan(data.x(i,index))
            replot_check = 1;
        else
            replot_check = 0;
        end
    ...
    % peak file selected
    if isnan(data.x(i,index)) && replot_check == 0 && marker == 1 
        exitwhile = 0;
    elseif ~isnan(data.x(i,index)) && marker == 1
        prompt_peak_cur = sprintf('The %s%d peak has already been selected at %d dB SPL during this session. Would you like to replace peak selection?',type, id, spl(index));
    ...
    elseif replot_check == 1 && marker == 1
        prompt_peak_old = sprintf('The %s%d peak has already been selected at %d dB SPL in a previous session. Would you like to replace peak selection?',type, id, spl(index));
        ...
        if contains(answer_peak_old,'Replace Selection')
            ...
            updated_data = data;
            updated_data.freq = freq;
            updated_data.x(i,index) = NaN;
            updated_data.y(i,index) = NaN;            
            replot = [];
            exitwhile = 0;
            stop = 1;
            cla;
