...
if ok ~= 0 & length(selections) >= 1 & length(selections) <=15
	...
	for i=1:numfiles %%% Sort the files by date
        ...
        unsorted(i,1)=i; unsorted(i,2)=str2num([str{selections(1,i)}(13:16) str{selections(1,i)}(18:19) str{selections(1,i)}(21:22)]);
	end
	...
	for i=1:numfiles %Load the selected data files in order by date
		...
		dd(i).data=load([abr_data_dir file(i,:)]);
		dd(i).name=strrep(file(i,:),'_',' ');
...
% 			end
		...
		elseif ~isfield(dd(i).data,'abrs') %%% Dummy matrices for plots
			dd(i).data.abrs.thresholds=NaN(1,4);
			dd(i).data.abrs.x=NaN(1,10);
			dd(i).data.abrs.y=NaN(1,12);
			
		end
		
		if ~isfield(dd(i).data,'dpoae') %%% Dummy matrix for plot
			dd(i).data.dpoae.data=NaN(1,4);
