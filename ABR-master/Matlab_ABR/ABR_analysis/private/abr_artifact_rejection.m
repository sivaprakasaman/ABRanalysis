...
if strcmp(continueButton,'Yes')    
    ...
    for aNum = 1:length(a_files)
        ...
        %Save new file called arawname with x_araw
        %Copied below from gain_20000 code -- MAKE THIS INTO A FUNCTION????
        [dummy1 dummy2 ext] = fileparts(fname_araw);
        if (strcmp(ext,'.m') ~= 1)
            fname = [fname_araw '.m'];
        end
        fid = fopen(fname_araw,'wt+');
        if (fid < 0)
            rc = -1;
            return;
        end      
        [dirname filename] = fileparts(fname_araw);
       ...
       cd(here1);
       %Now araw file has been saved into folder
       ...
       %Save new file called aname with x.AD_Data.AD_Avg_V corrected
       %Copied below from gain_20000 code -- MAKE THIS INTO A FUNCTION????
       [dummy1 dummy2 ext] = fileparts(aname);
       if (strcmp(ext,'.m') ~= 1)
           fname = [aname '.m'];
       end
       fid = fopen(aname,'wt+');
       ...
       [dirname filename] = fileparts(aname);
       fprintf(fid,'function x = %s\n', filename);
       here1 = pwd;
